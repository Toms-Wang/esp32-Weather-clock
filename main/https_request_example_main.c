...
static void https_get_request(esp_tls_cfg_t cfg)
{
    ...
    do
    {
        ...
        if (str_1 != NULL)
        {
            ...
            ESP_LOGI(TAG, "天气情况测试；");
            uint8_t test[] = "保中";
            ESP_LOGI(TAG, "%x %x %x %x %d", test[0], test[1], test[2], test[3], sizeof(test));
            ...
            if (pJsonRoot != NULL)
            {
                ...
                char wea[10] = {0};
                char tem[10] = {0};
                //cJSON *pMacAdress = cJSON_GetObjectItem(pJsonRoot, "results"); // 解析mac字段字符串内容
                ...
                if(json_results)
                {
                	...
                	if(re_child)
                	{
                		...
                		if(json_now)
						{
							...
							strcpy(wea, json_text -> valuestring);
//							ESP_LOGI(TAG, "天气：%s", wea);
							...
							strcpy(tem, json_temerature -> valuestring);
//							ESP_LOGI(TAG, "温度：%s", tem);
						}
                ...
                LCD_showString(16, 32, "tem:", RED);
                LCD_showString(48, 32, tem, RED);
            }
        }
    } while (1);
